...
std::vector<std::vector<int>> create_threshold_matrix_from_noise(Noise2D<int> noise, size_t width, size_t height);
...
std::string generate_white_noise(int width, int height, int output_levels, bool fourier, bool benchmark)
{
    ...
    {
        time_end = clock();
        output += std::to_string(1000.0 * (time_end - time_start) / CLOCKS_PER_SEC) + " ms\n";
    }

    std::vector<std::vector<int>> threshold_matrix = create_threshold_matrix_from_noise(white_noise, width, height);
...
std::vector<std::vector<int>> create_threshold_matrix_from_noise(Noise2D<int> noise, size_t width, size_t height)
{
    ...
    for(size_t y = 0; y < height; y++)
    {
        ...
        {
            threshold_matrix[y][x] = noise.get_noise_at(x, y);
        }
    }
